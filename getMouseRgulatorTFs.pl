...
my $total_gene = 23481 + 1593; 			# the total number of non-TF target genes.
...
my $target_gene_file = "./data/Mouse.target.genes.txt";
my $gene_symbl_file = "./data/Mouse.gene.symbls.txt";
my $edge_file_1 = "./data/Mouse.SigEdges.part1.txt"; 		# with singinificant interacting TF-target gene pair.
my $edge_file_2 = "./data/Mouse.SigEdges.part2.txt"; 		# with singinificant interacting TF-target gene pair.
...
unless( -e $edge_file_1 ){
	print "The file 'Mouse.SigEdges.part1.txt' not found. It should be saved in the 'data' sub-directory. And the location of the script 'getMouseRegulatorTFs.pl' should not be changed.\nExit in 5 second.\n";
	sleep 5;
	exit 1;
}

unless( -e $edge_file_2 ){
	print "The file 'Mouse.SigEdges.part2.txt' not found. It should be saved in the 'data' sub-directory. And the location of the script 'getMouseRegulatorTFs.pl' should not be changed.\nExit in 5 second.\n";
	sleep 5;
	exit 1;
}

unless( -e $gene_symbl_file ){
	print "The file 'Mouse.gene.symbls.txt' not found. It should be saved in the 'data' sub-directory. And the location of the script 'getArabidopsisRegulatorTFs.pl' should not be changed.\nExit in 5 seconds.\n";
	sleep 5;
	exit 1;
}

unless( -e $target_gene_file ){
	print "Target gene file 'Mouse.target.genes.txt' not found. It should be saved in the 'data' sub-directory. And the location of the script 'getArabidopsisRegulatorTFs.pl' should not be changed.\nExit in 5 seconds.\n";
	sleep 5;
	exit 1;
}
...
while ( $l = <IN>){
	...
	$t[0] = uc ($t[0]);
	#$t[0] = substr($t[0],0,9);
	if ( $seen{$t[0]}){				# only consider target genes listed in ./data/Mouse.target.genes.txt 
